...
async function loadPrices() {
    ...
    tbody.innerHTML = data.map(p => `
        <tr>
            ...
            <td><button class="btn-action btn-primary" onclick="savePrice(${p.id})">Save</button></td>
        </tr>
    `).join('');
}
...
async function savePrice(id) {
    ...
    if (res && res.message === 'Price updated') { toast('Price updated!'); loadPrices(); }
    else toast('Failed to update price');
}
